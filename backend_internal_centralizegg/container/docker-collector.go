...
func (dc *DockerCollector) collectOne(s data_centralizegg.GenericServer) error {
	...
	diskIORaw, err := dc.runCommand(client, `awk '/(sd[a-z]+|nvme[0-9]n[0-9]+|vd[a-z]+|xvd[a-z]+)$/ {print $3, $6, $10}' /proc/diskstats`)
	if err == nil {
...
func (dc *DockerCollector) GetHostLogs(id int64) (string, error) {
	...
	return dc.runCommand(client, "journalctl -n 50 --no-pager")
}
